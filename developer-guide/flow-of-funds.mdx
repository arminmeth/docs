...
sequenceDiagram
	...
	u-->>c: On-Ramping Transfer
	c-->>s: Incoming Transfer Notification
	s-->>c: Instant Settlement Approval
...
<Steps>
	<Step> 
		User initiates on-ramping transfer to Developer. 
	...
	<Step>
		Developer sends incoming transfer notification to Superbank.
	</Step>
	<Step>
		Superbank approves instant settlement request.
	</Step>
	<Step>
		Developer settles on-chain transfer.
	...
	<Step>
		On-ramping transfer is completed after `T0+X`.
...
<Steps>
	<Step> 
		User initiates off-ramping transfer to Developer. 
	...
	<Step>
		Developer settles off-chain transfer.
	...
	<Step>
		Off-ramping transfer is completed after `T0+X`.

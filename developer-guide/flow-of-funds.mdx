...
1. Developer forwards Instant Settlement Capital to User. = T0
   1. `destination`: User’s wallet
1. Fiat Transfer from User to Developer is completed after T + X (initiated in Step 1). = T0 + X
   1. `source`: User’s bank account
   2. `destination`: Developer’s bank account
2. Developer forwards received Fiat Transfer funds + Fees to Superbank. = T0 + X
   1. `source`: Developer’s bank account
...
2. Superbank provides Instant Settlement Capital to Developer in USDC/T.
   1. `source`: Superbank’s pre-funded wallet
   2. `destination`: Developer’s wallet
1. Developer forwards Instant Settlement Capital to User. **[T0]** **[INSTANT SETTLEMENT]**
